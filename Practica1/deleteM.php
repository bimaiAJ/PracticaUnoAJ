...
<div class="container jumbotron cambio">
    <h2>Eliminación de Maestro</h2>
    <form id="deleteTeacherForm"> <!-- Asignamos un ID al formulario para el manejo con Ajax -->
        <div>
        ...
        <div class="lead">
            <input type="hidden" value="6" name="type"> <!-- Tipo de operación para identificar eliminación -->
            <button type="submit" class="btn btn-danger">Borrar</button> <!-- Usamos botón en lugar de submit -->
        </div>
    </form>
...
document.getElementById('deleteTeacherForm').addEventListener('submit', function(event) {
    ...
    .then(data => {
        ...
        if (data.success) {
            responseDiv.innerHTML = '<div class="alert alert-success">Maestro borrado con éxito.</div>';
            setTimeout(() => {
    ...
    .catch(error => {
        // Manejar errores
        document.getElementById('responseMessage').innerHTML = '<div class="alert alert-danger">Error al borrar el maestro.</div>';
    });
});
</script>
