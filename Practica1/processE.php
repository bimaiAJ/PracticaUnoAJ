...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['type'] == 1) { // Creación de estudiante
        $id = $_POST['id'];
        ...
        $sql = "INSERT INTO student (identifier, First_Name, Second_Name, Last_Name, Sec_Last_Name, Birth_Date)
                VALUES ($id, '$first_name', '$second_name', '$last_name', '$sec_last_name', '$birth_date')";

        if (mysqli_query($conn, $sql)) {
            $response['success'] = true;
            $response['message'] = 'Nuevo estudiante creado con éxito.';
        } else {
            $response['message'] = 'Error: ' . mysqli_error($conn);
        }
        
    } else if ($_POST['type'] == 3) { // Actualización de estudiante
        $id = $_POST['id'];
        $first_name = $_POST['firstName'];
        $second_name = isset($_POST['secondName']) ? $_POST['secondName'] : '';
        $last_name = $_POST['lastName'];
    ...
    } else if ($_POST['type'] == 5) { // Eliminación de estudiante
        $id = $_POST['id'];

        $sql = "DELETE FROM student WHERE identifier = $id";

        if (mysqli_query($conn, $sql)) {
            $response['success'] = true;
            $response['message'] = 'Estudiante borrado con éxito.';
        } else {
            $response['message'] = 'Error: ' . mysqli_error($conn);
        }
    }
